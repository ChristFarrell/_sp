...
// =========================================================
typedef enum {
    TK_FUNC, TK_RETURN, TK_IF, TK_WHILE, 
    TK_ID, TK_NUM, 
...
// =========================================================
int t_idx = 0;
void new_temp(char *s) { sprintf(s, "t%d", ++t_idx); }
void expression(char *res);
void statement();

void factor(char *res) {
    if (cur_token.type == TK_NUM) {
        new_temp(res); emit("IMM", cur_token.text, "-", res); next_token();
        ...
        if (cur_token.type == TK_LPAREN) { 
            ...
            next_token();
            new_temp(res); char c_str[10]; sprintf(c_str, "%d", count);
...
void term(char *res) {
    ...
    while (cur_token.type == TK_MUL || cur_token.type == TK_DIV) {
        ...
        next_token(); factor(r); new_temp(t); emit(op, l, r, t); strcpy(l, t);
    }
    strcpy(res, l);
}
...
void arith_expr(char *res) {
    ...
    while (cur_token.type == TK_PLUS || cur_token.type == TK_MINUS) {
        ...
        next_token(); term(r); new_temp(t); emit(op, l, r, t); strcpy(l, t);
    }
    strcpy(res, l);
}
...
void expression(char *res) {
    ...
    if (cur_token.type == TK_EQ || cur_token.type == TK_LT || cur_token.type == TK_GT) {
        ...
        new_temp(t); emit(op, l, r, t); strcpy(res, t);
    } else {
        strcpy(res, l);
    }
}
...
void vm_execute() {
    ...
    // Scan for function entry points
...
int main(int argc, char *argv[]) {
    // Set terminal to UTF-8 for better character handling
    #ifdef _WIN32
    ...
    src = buffer;
    printf("Generated Intermediate Code (PC: Quadruples):\n");
    printf("--------------------------------------------\n");
